...
class WebsiteCrawler:
    ...
    async def scrape_website(self, url, tags, languages):
            ...
            try:
                ...
                logger.info(f"传入的标签：{tags}")
                logger.info(f"传入的语言：{languages}")

                # 网页访问前增加日志
                logger.info(f"准备访问网页: {url}")
                ...
                logger.info("准备调用 default_file_key 方法")
                try:
                    image_key = oss.default_file_key(url)
                    logger.info(f"成功生成 image_key: {image_key}")
                except Exception as e:
                    logger.error(f"调用 default_file_key 方法失败: {e}")
                    raise
                ...
                # 使用llm工具处理content
                logger.info("开始使用LLM处理网页内容")
                detail = llm.process_detail(content)
                
                if not detail:
                    logger.error("无法生成网页详情")
                    return None
                ...
                if tags and detail:
                    ...
                    logger.info(f"处理后的标签: {processed_tags}")
                ...
                logger.error(f"爬取网站 {url} 发生异常：{str(e)}")
